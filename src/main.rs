...
/// A three-headed guard for Claude Code's Bash tool
///
...
/// decisions, where a command is only bad because of state nothing in the
/// command line reveals).
///
/// guard is the one command wired into PreToolUse. It runs the fail-closed
/// gate first, then whichever heads are enabled in
...
#[derive(Subcommand)]
enum Command {
    /// Judge one Bash command (the PreToolUse hook)
    ///
    ...
    ///
    /// Denies all Bash while a head is degraded. Always runs first inside
    /// guard; running it directly is mainly for debugging.
    Gate,
